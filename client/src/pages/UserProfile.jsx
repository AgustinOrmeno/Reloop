...
function FavoritesTab() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-48">
      <p className="text-sm text-muted">Cargando favoritos...</p>
    </div>
  )

  if (favorites.length === 0) return (
    <div className="flex items-center justify-center h-48">
      <p className="text-sm text-muted">No tenés prendas en favoritos todavía.</p>
    </div>
  )
        ...
        <div className="flex items-start justify-between mb-12 pb-12 border-b border-carbon/10">
          ...
          <div className="flex gap-8">
            <div className="text-center">
              <div className="font-display text-3xl font-bold text-carbon">
                {user.rating || 0}
              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-8">
              <p className="text-sm text-muted">
                {user.products?.length || 0} prendas publicadas
              </p>
              <Link
                to="/publicar"
            ...
            {user.products?.length === 0 ? (
              <div className="flex items-center justify-center h-48">
                <p className="text-sm text-muted">
                  No tenés prendas publicadas todavía.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-4 gap-5">
