        ...
        <div className="grid grid-cols-3 gap-16">
          ...
          <form onSubmit={handleSubmit} className="col-span-2 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-medium tracking-widest uppercase text-muted block mb-3">Ciudad</label>
                ...
                {sizes.map((size) => (
                  <button key={size} type="button" onClick={() => setSelectedSize(size)}
                    className={`text-xs font-medium px-4 py-2 rounded border transition-colors duration-200 ${
                      selectedSize === size ? 'bg-carbon text-white border-carbon' : 'bg-white text-carbon border-carbon/20 hover:border-terracota hover:text-terracota'
                    }`}>
                    {size}
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="flex gap-3">
                {conditions.map((cond) => (
                  <button key={cond} type="button" onClick={() => setSelectedCondition(cond)}
                    className={`text-xs font-medium px-4 py-2.5 rounded border transition-colors duration-200 ${
                      selectedCondition === cond ? 'bg-carbon text-white border-carbon' : 'bg-white text-carbon border-carbon/20 hover:border-terracota hover:text-terracota'
                    }`}>
                    {cond}
                  </button>
                ))}
            ...
            <div className="bg-carbon rounded-md p-6">
              ...
              <div className="flex flex-col gap-4">
                {[
                  ...
                  { icon: '💰', tip: 'Investigá precios similares antes de publicar' },
                  { icon: '📍', tip: 'Indicá si aceptás entrega en mano' },
                ].map((item) => (
                  <div key={item.tip} className="flex items-start gap-3">
                    <span className="text-base">{item.icon}</span>
                    <p className="text-xs font-light text-white/55 leading-relaxed">{item.tip}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="border border-carbon/10 rounded-md p-6">
              <p className="text-xs font-medium tracking-widest uppercase text-muted mb-3">Comisión</p>
              <p className="font-display text-4xl font-bold text-carbon mb-2">0%</p>
              <p className="text-xs text-muted leading-relaxed">En Reloop no cobramos comisión. Lo que ponés es lo que recibís.</p>
            </div>
          </div>
