...
export default function ProductCard({ product }) {
  return (
    <Link to={`/producto/${product.id}`} className="block cursor-pointer transition-transform duration-200 hover:-translate-y-1">
      ...
      <div className={`${product.bg || 'bg-terracota'} w-full aspect-[3/4] rounded-md mb-3 relative overflow-hidden`}>
        <button
      ...
      <p className="text-xs text-muted mb-2">{product.brand || product.user?.name} · {product.city}</p>
