...
export default function ProductCard({ product, initialFavorited = false }) {
  ...
  const handleFavorite = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await userService.toggleFavorite(product.id)
      ...
      <div className="w-full aspect-[3/4] rounded-md mb-3 relative overflow-hidden bg-beige-dark">
        ...
        >
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className={`w-4 h-4 transition-colors duration-200 ${favorited ? 'fill-terracota stroke-terracota' : 'fill-none stroke-carbon'}`}
          >
